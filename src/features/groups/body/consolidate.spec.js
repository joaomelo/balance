...
describe('groups balance consolidation', () => {
  test('produce one consolidation data object for every group', () => {
    const consolidatedGroups = consolidateGroupsBalances(groups, accounts, balances);
  ...
  test('produce one group balance for every date with a relate account balance', () => {
    ...
    const liquidBalances = extractBalancesByName('liquid', groups, consolidatedGroups);
    expect(liquidBalances).toHaveLength(6);

    const assetsBalances = extractBalancesByName('assets', groups, consolidatedGroups);
    expect(assetsBalances).toHaveLength(1);

    const investmentsBalances = extractBalancesByName('investments', groups, consolidatedGroups);
  ...
  test('groups balances are consolidated accordingly', () => {
    ...
    const extractLiquid = dateStr => extractBalance('liquid', dateStr, groups, consolidated);
    ...
    const balanceAssets1001 = extractBalance('assets', '2021-10-01', groups, consolidated);
